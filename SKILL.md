...
pi.registerTool({
  ...
  promptGuidelines: ["Call comsol_greet to…"],      // 必须提及工具名的指导
...
要点：`prepareArguments(args)` 兼容旧会话；`renderCall/renderResult` 自定义 TUI；execute 抛错 = 失败；返回 `terminate: true` 停止后续 LLM 调用；可用同名覆盖内置工具（read/bash/edit/write）。
...
- `registerProvider(name, config)` — 动态 provider（含 OAuth、refreshModels、streamSimple）
...
已知命令（以 `rpc-types.ts` 为准）：
`get_state` / `get_messages` / `send_message` / `new_session` / `switch_session`（传 sessionPath）/ `set_session_name`（name）/ `fork` / `compact` / `interrupt` / `list_sessions` / 以及各内置工具调用。
...
- `agegr/pi-web` — Pi Agent 的 Web UI（与本仓库同类，可参考 RPC 对接）
...
## 9. 本仓库（COMSOL Agent）的对接约定

- 扩展：`packages/coding-agent/src/extensions/comsol/`（tools/hooks/guards/adapters/schemas，`pi.registerTool`）
- 会话身份单一权威 = Pi `get_state` 返回值；列表/切换/新建/命名走原生 RPC（`list_sessions`/`switch_session`/`new_session`/`set_session_name`）
- 验收：`git diff coding-agent/src -- ':!extensions'` 为空（内核零改动）
- 详见 `docs/development/pi-agent-foundation-plan.md` 与 AGENTS.md §21
